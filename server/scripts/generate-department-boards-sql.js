/**
 * migrate-department-boards.sql + 학과별 boards/게시글 시드 SQL 생성
 * 출력: server/scripts/seed-department-boards-full.sql
 * Supabase Dashboard → SQL Editor 에서 한 번 실행
 */
const fs = require('fs');
const path = require('path');

const MIGRATE_SQL = fs.readFileSync(
  path.join(__dirname, 'migrate-department-boards.sql'),
  'utf8',
);
...
const output = `-- 학과별 게시판 마이그레이션 + 시드 (한 번 실행)
...
const outPath = path.join(__dirname, 'seed-department-boards-full.sql');
fs.writeFileSync(outPath, output, 'utf8');
console.log(`Wrote ${outPath} (${output.length} bytes)`);

// sanity: keep JS templates in sync (unused at runtime but documents parity)
void buildPostsForDepartment;
void BOARD_SPECS;
void sqlLiteral;
