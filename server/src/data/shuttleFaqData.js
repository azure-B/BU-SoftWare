/** 통학·캠퍼스 셔틀 FAQ용 운행 데이터 (front shuttleData.js 와 동일 기준) */

const CAMPUS_SHUTTLE_SCHEDULE = [
  {
    direction: '등교',
    location:
  ...
  },
  {
    ...
    trips: [
      ...
      },
      {
        ...
        days: '월~목',
        time: '16:30',
        stops: ['학교승차장', '신갈TG', '영통입구', '법원사거리', '범계역', '안양역'],
        note: null,
      },
    ],
  },
    ...
    trips: [
      {
        ...
        note: '07:30 터미널, 07:35 LG전자, 07:45 배방역 / 승차권: 진리관 CU편의점 앞 발권기',
      },
      {
        direction: '하교',
        days: '월~금',
...
function formatCampusShuttleAnswer() {
  const lines = ['【캠퍼스 셔틀버스】'];
  CAMPUS_SHUTTLE_SCHEDULE.forEach((trip) => {
    lines.push(`${trip.direction} (${trip.days}): ${trip.time}`);
    lines.push(`  승·하차: ${trip.location}`);
    if (trip.note) lines.push(`  경로: ${trip.note}`);
  });
  lines.push('자세한 승차 위치는 백석대학교 홈페이지 공지를 참고해 주세요.');
  return lines.join('\n');
}

function formatRouteAnswer(route) {
  const lines = [`【${route.name} 통학버스】`, `요금: ${route.fare}`];
  route.trips.forEach((trip) => {
    lines.push(`${trip.direction} (${trip.days}): ${trip.time}`);
    if (trip.stops.length > 0) {
      lines.push(`  정류장: ${trip.stops.join(' → ')}`);
    }
    if (trip.note) lines.push(`  참고: ${trip.note}`);
  });
  return lines.join('\n');
}

function formatCommuterOverview() {
  const routeNames = COMMUTER_ROUTES.map((route) => route.name).join(', ');
  return [
    '【통학버스 안내】',
    `운행 노선: ${routeNames}`,
    '노선명(예: 교대, 아산, 분당)을 포함해 질문하면 상세 시간표를 알려드립니다.',
    '캠퍼스 셔틀은 "캠퍼스 셔틀" 또는 "두정역"으로 물어봐 주세요.',
    '아산 노선 승차권은 진리관 CU편의점 앞 발권기를 이용해 주세요.',
  ].join('\n');
}
